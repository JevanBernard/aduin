...
export default function HeatmapMap({
  ...
  return (
    ...
    >
      ...
      <div
        className="shrink-0 px-3 py-2 flex flex-col gap-1.5 z-10"
        style={{
      ...
      >
        ...
        <div className="flex flex-wrap gap-1.5">
          {(kategoriFilter || ["Semua", "Infrastruktur", "Lingkungan", "Bencana", "Air & Sanitasi", "Transportasi", "Ekonomi", "Keamanan", "Pendidikan", "Kesehatan", "Pelayanan Publik"]).map((k) => (
            <button
              key={k}
              onClick={() => onFilterKat(k)}
              className="px-2.5 py-1 rounded-md text-xs font-semibold font-raleway transition"
              style={{
                background: filterKat === k ? (dark ? "#e2e8f0" : "#0F172A") : (dark ? "#1e293b" : "#f1f5f9"),
        ...
        <div className="flex gap-1.5">
          {URGENSI_FILTER.map((u) => (
            <button
              ...
              className="px-2.5 py-0.5 rounded text-xs font-semibold font-raleway transition"
              style={{
                background: filterUrg === u.label ? u.color : (dark ? "#1e293b" : "#f1f5f9"),
      ...
      <div className="flex-1 relative">
            ...
            return (
              <div key={`${kab.name}-${i}`}>
                <CircleMarker
                  ...
                  radius={kab.size * 0.9}
                  pathOptions={{ fillColor: glowColor, fillOpacity: 0.4, stroke: false }}
                  interactive={false}
                />
                <CircleMarker
                  center={[kab.lat, kab.lng]}
                  ...
                  eventHandlers={{ click: () => onSelectWilayah(i) }}
                >
                  <Tooltip direction="top" offset={[0, -kab.size * 0.4]} permanent={true} className="custom-tooltip">
                    <div style={{ textAlign: "center", fontFamily: "Raleway, sans-serif", background: "transparent", border: "none", boxShadow: "none" }}>
                      <div style={{ fontSize: kab.size > 35 ? 14 : kab.size > 25 ? 12 : 10, fontWeight: 800, fontFamily: "Inter, sans-serif", color: dark ? "#e2e8f0" : "#1e293b" }}>
                        {kab.count}
                      </div>
                      <div style={{ fontSize: 10, fontWeight: 700, color: dark ? "#94a3b8" : "#475569", marginTop: 1 }}>
                        {kab.name}
                      </div>
                    </div>
                  </Tooltip>
        ...
        >
          <p className="text-[10px] font-bold mb-1 font-raleway" style={{ color: dark ? "#e2e8f0" : "#1E293B" }}>Keterangan</p>
