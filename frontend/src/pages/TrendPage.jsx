...
export default function TrendPage() {
  ...
  const periodKey = period.type === "preset" ? period.value : "30d";
  const trendData = MOCK_TREND_LINE[periodKey] || MOCK_TREND_LINE["30d"];
  ...
  const handleExportPDF = async () => {
    if (!contentRef.current) {
      alert("Area konten tidak ditemukan");
      return;
    }
    setExporting(true);
    setExportMenu(false);

    // Tunggu sebentar agar dropdown tertutup dulu
    ...
    try {
      ...
      pdf.text(
        `Periode: ${trendData.subtitle} | Diekspor: ${new Date().toLocaleDateString("id-ID")}`,
        14, 22
      );
      ...
      pdf.addImage(imgData, "PNG", 14, 28, imgWidth, finalHeight);
          ...
          <header className="flex flex-wrap items-start justify-between gap-4 mb-3">
            ...
            <div className="flex items-center gap-2.5">
              <div className="relative w-72">
                <svg width="20" height="20" viewBox="0 0 24 24" className="absolute left-3.5 top-1/2 -translate-y-1/2" style={{ color: "#626262" }}><circle cx="11" cy="11" r="8" stroke="currentColor" strokeWidth="2" fill="none"/><path d="M21 21l-4.35-4.35" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>
                <input type="search" placeholder="Cari Laporan, Lokasi...." className="w-full pl-11 pr-4 py-3 rounded-lg text-sm font-semibold font-raleway outline-none" style={{ background: dark ? "rgba(255,255,255,0.06)" : "rgba(164,158,158,0.2)", color: dark ? "#e2e8f0" : "#333" }} />
              </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
            ...
            <div className="relative" ref={exportMenuRef}>
              ...
              >
                ...
                {exporting ? "Mengekspor..." : "Export Excel/PDF"}
              ...
              {exportMenu && (
                <div className="absolute top-full left-0 mt-1 rounded-lg overflow-hidden z-50" style={{ background: dark ? "#1e293b" : "#fff", border: `1px solid ${dark ? "#334155" : "#e2e8f0"}`, boxShadow: "0 8px 24px rgba(0,0,0,0.12)", minWidth: 200 }}>
                  <button onClick={handleExportCSV} className="flex items-center gap-3 w-full px-4 py-3 text-sm font-raleway font-semibold text-left transition hover:opacity-80" style={{ color: dark ? "#e2e8f0" : "#333" }}>
                    <span style={{ color: "#1d6f42" }}>📊</span> Export CSV (Excel)
                  </button>
                  <button onClick={handleExportPDF} className="flex items-center gap-3 w-full px-4 py-3 text-sm font-raleway font-semibold text-left transition hover:opacity-80" style={{ color: dark ? "#e2e8f0" : "#333", borderTop: `1px solid ${dark ? "#334155" : "#f1f5f9"}` }}>
            ...
            <section className="rounded-2xl p-6 mb-5" style={{ background: cardBg, boxShadow: cardShadow, border: cardBorder }}>
              ...
              <ResponsiveContainer width="100%" height={320}>
                <LineChart data={chartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                  ...
                  <Tooltip contentStyle={{ background: dark ? "#1e293b" : "#fff", border: `1px solid ${dark ? "#334155" : "#e2e8f0"}`, borderRadius: 8, fontFamily: "Raleway", fontSize: 13, boxShadow: "0 4px 12px rgba(0,0,0,0.1)" }} labelStyle={{ fontWeight: 700, color: textPrimary }} itemStyle={{ color: textSecondary }} />
                  <Legend wrapperStyle={{ fontFamily: "Inter", fontSize: 12, color: textSecondary }} />
                  {trendData.datasets.map((ds) => (
                    <Line key={ds.name} type="monotone" dataKey={ds.name} stroke={ds.color} strokeWidth={2.5} dot={{ r: 4, fill: ds.color, stroke: "#fff", strokeWidth: 2 }} activeDot={{ r: 6, fill: ds.color, stroke: "#fff", strokeWidth: 2 }} />
                  ))}
                </LineChart>
              </ResponsiveContainer>
            </section>
