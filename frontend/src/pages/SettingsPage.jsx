...
import { ThemeToggle, ProfileDropdown } from "../components/common/Header";
...
const TABS = [
  ...
  { key: "wilayah", label: "Wilayah & Koordinat" },
...
export default function SettingsPage() {
  ...
  const handleReset = () => {
    if (window.confirm("Reset semua pengaturan ke default?")) {
      setFormData({
        namaInstansi: "", wilayahCakupan: "", emailAdmin: "",
        telepon: "", alamat: "", notifUrgent: true, notifLaporanBaru: false, notifEmail: true,
      });
    }
  };
            ...
            <nav className="lg:w-48 shrink-0 flex lg:flex-col gap-1 overflow-x-auto lg:overflow-visible pb-2 lg:pb-0">
              {TABS.map((tab) => (
                <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                  ...
                  style={{
                    background: activeTab === tab.key ? dark ? "rgba(62,139,243,0.15)" : "rgba(62,139,243,0.08)" : "transparent",
...
function InputField({ label, value, onChange, dark, inputBg, inputBorder, textPrimary, type = "text", textarea = false }) {
  return (
    <div>
...
function KategoriTab({ dark, textPrimary, textSecondary, inputBg, inputBorder }) {
  ...
  useEffect(() => {
    getKategoriSettings().then((res) => {
      ...
      setKategoriList(["Infrastruktur", "Lingkungan", "Air & Sanitasi", "Bencana", "Transportasi", "Pelayanan Publik", "Ekonomi", "Keamanan", "Pendidikan", "Kesehatan"]);
    });
  }, []);
    ...
    <>
      ...
      <div className="flex gap-2 mb-5 max-w-xl">
        <input value={newKategori} onChange={(e) => setNewKategori(e.target.value)}
          onKeyDown={(e) => e.key === "Enter" && handleAdd()} placeholder="Tambah kategori baru..."
...
function DinasTab({ dark, textPrimary, textSecondary, inputBg, inputBorder }) {
  ...
  const handleSave = async () => {
    ...
    } catch { alert("Gagal menyimpan daftar dinas"); }
    finally { setSaving(false); }
  };
...
function WilayahTab({ dark, textPrimary, textSecondary, inputBg, inputBorder }) {
  ...
  const getToken = () => {
    try { return JSON.parse(localStorage.getItem("aduin_user"))?.token; }
    catch { return null; }
  };
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    try {
      const kecamatanArr = newKecamatan.split(",").map(k => k.trim()).filter(Boolean);
  ...
  const handleDelete = async (id) => {
    ...
    await fetch(`${API_URL}/wilayah/${id}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${getToken()}` }
    });
  ...
  const handleAddKecamatan = async (wilayah, kecBaru) => {
    ...
    await fetch(`${API_URL}/wilayah/${wilayah.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${getToken()}` },
  ...
  return (
    <>
      ...
      <p className="text-sm font-raleway mb-6" style={{ color: textSecondary }}>Koordinat untuk heatmap. Kecamatan untuk dropdown form warga.</p>
      ...
      <div className="flex flex-col gap-3 max-w-2xl">
        {wilayahList.map((w) => (
          <div key={w.id} className="rounded-xl overflow-hidden" style={{ border: `1px solid ${inputBorder}` }}>
            <div className="flex items-center justify-between px-4 py-3" style={{ background: dark ? "#0f172a" : "#f8fafc" }}>
              <div>
                <span className="text-sm font-bold font-raleway" style={{ color: textPrimary }}>{w.nama}</span>
                <span className="text-xs font-raleway ml-3" style={{ color: textSecondary }}>{w.latitude}, {w.longitude}</span>
                <span className="text-xs font-raleway ml-2" style={{ color: textSecondary }}>· {w.kecamatan?.length || 0} kecamatan</span>
              </div>
              <div className="flex items-center gap-3">
                <button onClick={() => setExpandedId(expandedId === w.id ? null : w.id)}
                  className="text-xs font-bold font-raleway" style={{ color: "#3e8bf3" }}>
                  {expandedId === w.id ? "Tutup" : "Edit Kecamatan"}
                </button>
                <button onClick={() => handleDelete(w.id)} className="text-xs font-raleway text-red-500 hover:text-red-700">Hapus</button>
              </div>
            </div>
            ...
            {expandedId === w.id && (
              <div className="px-4 py-3" style={{ background: dark ? "#1e293b" : "#fff", borderTop: `1px solid ${inputBorder}` }}>
                ...
                <AddKecamatanInline onAdd={(kec) => handleAddKecamatan(w, kec)}
...
function AddKecamatanInline({ onAdd, inputBg, inputBorder, textPrimary }) {
