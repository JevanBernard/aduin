...
export default function LaporanPage() {
  ...
  // Ganti mock dengan state dari API
  ...
  // Fetch dari API setiap kali filter/page berubah
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        setRawData(MOCK_LAPORAN); // fallback ke mock kalau API error
  ...
  const paginatedData = rawData; // sudah dipaginasi dari backend

  // Format data dari API ke format yang dipakai tabel
  const formattedData = paginatedData.map((r) => ({
    id: r.reportNumber || r.id,
    text: r.text,
    kategori: r.categories?.[0] || "-",
    urgensi: r.urgensi === "tinggi" ? "Tinggi" : r.urgensi === "sedang" ? "Sedang" : r.urgensi === "rendah" ? "Rendah" : "-",
    score: r.priorityScore ? Math.round(r.priorityScore) : 0,
    status: r.status === "DITERIMA" ? "Belum"
          ...
          <header className="flex flex-wrap items-start justify-between gap-4 mb-3">
            ...
            <div className="flex items-center gap-2.5">
              <div className="relative w-72">
                ...
                <input type="search" placeholder="Cari Laporan, Lokasi...." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-11 pr-4 py-3 rounded-lg text-sm font-semibold font-raleway outline-none" style={{ background: dark ? "rgba(255,255,255,0.06)" : "rgba(164,158,158,0.2)", color: dark ? "#e2e8f0" : "#333" }} />
              </div>
