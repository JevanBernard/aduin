...
import { getReportByNumber, updateReportStatus, deleteReport } from "../services/api";
import { useDinas } from "../hooks/useSettings";
...
export default function LaporanDetailPage() {
  ...
  const { dinas: dinasList } = useDinas();
  const DINAS_OPTIONS = ["Pilih Dinas...", ...dinasList.map((d) => d.nama)];
  ...
  useEffect(() => {
    async function fetchDetail() {
      setLoadingData(true);
      try {
        const res = await getReportByNumber(id); // id dari useParams = reportNumber
        const r = res.data;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Refresh riwayat
      const res = await getReportByNumber(id);
      setData((prev) => ({
        ...prev,
                ...
                {lightboxPhoto && (
                  ...
                  >
                    <div className="relative max-w-3xl w-full" onClick={(e) => e.stopPropagation()}>
                      ...
                      <button
                        onClick={() => setLightboxPhoto(null)}
                        className="absolute top-3 right-3 w-9 h-9 rounded-full flex items-center justify-center text-white transition hover:opacity-80"
                      ...
                      <p className="text-center text-xs text-white mt-3 opacity-60">Klik di luar untuk menutup</p>
                    </div>
                  </div>
                )}
              </section>
              ...
              <div className="mb-5">
                ...
                <div className="relative">
                  <select value={disposisi} onChange={(e) => setDisposisi(e.target.value)}
                    ...
                    {DINAS_OPTIONS.map((d) => (<option key={d} value={d === "Pilih Dinas..." ? "" : d}>{d}</option>))}
                  </select>
                  <svg width="16" height="16" viewBox="0 0 24 24" className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none" stroke={textSecondary} strokeWidth="2" fill="none" strokeLinecap="round">
                    <path d="M6 9l6 6 6-6"/>
                  </svg>
