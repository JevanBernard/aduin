...
async function getTrends(req, res, next) {
  try {
    const { kategori, wilayah, status } = req.query;
    ...
    if (period === "7d") {
      ...
      // Custom range — bagi jadi 7 bucket
      const startMs = new Date(start).getTime();
      const endMs = new Date(end).getTime();
      const bucketSize = (endMs - startMs) / 7;
      for (let i = 0; i < 7; i++) {
        const bucketStart = new Date(startMs + i * bucketSize);
        const bucketEnd = new Date(startMs + (i + 1) * bucketSize);
        labels.push(bucketStart.toLocaleDateString("id-ID", { day: "numeric", month: "short" }));
        buckets.push({ start: bucketStart, end: bucketEnd });
      }
    }

    // Query per bucket
    const totalData = await Promise.all(buckets.map(b =>
      prisma.report.count({ where: { ...baseFilter, createdAt: { gte: b.start, lte: b.end } } })
    ));
    const urgentData = await Promise.all(buckets.map(b =>
      prisma.report.count({ where: { ...baseFilter, urgensi: "tinggi", createdAt: { gte: b.start, lte: b.end } } })
    ));
    const selesaiData = await Promise.all(buckets.map(b =>
      prisma.report.count({ where: { ...baseFilter, status: "SELESAI", createdAt: { gte: b.start, lte: b.end } } })
    ));
...
// GET /api/dashboard/trends-bulanan
async function getTrendsBulanan(req, res, next) {
  try {
    const now = new Date();
    const results = [];
...
module.exports = { getStats, getHeatmap, getHeatmapDetail, getPriorities, getTrends, getDistribusi, getTopWilayah, getTrendsBulanan };